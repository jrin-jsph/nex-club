...
/**
 * This class builds the Admin Dashboard UI panel.
...
public class AdminDashboard {
    ...
    private String loggedInNid; // Field to store the current user's NID

    /**
     * Constructor: Builds the dashboard UI.
     * @param parentFrame The main application window (JFrame).
     * @param loggedInNid The NID of the user who just logged in.
     */
    public AdminDashboard(JFrame parentFrame, String loggedInNid) { 
        this.parentFrame = parentFrame;
        this.loggedInNid = loggedInNid; // Store the NID for this session
        buildContentPane();
    }
    ...
    private void buildContentPane() {
        ...
        prepareTables();
        loadDataFromDB(); // Will now load data specific to the loggedInNid

        dashboardTitleLabel.setText("Admin Control Panel");
    ...
    private void prepareTables() {
        ...
        try (Statement st = dbConn.createStatement()) {
            ...
            // The 'login' table is assumed to exist and is not created here.
        } catch (SQLException e) {
            showStatusMessage("Failed to prepare DB tables: " + e.getMessage(), true);
        }
    }
    ...
    private void loadDataFromDB() {
        ...
        // Use the loggedInNid that was passed during dashboard creation
        if (this.loggedInNid == null) {
             showStatusMessage("Error: No logged in user detected.", true);
             return;
        }
        
        String clubsSql = "SELECT * FROM clubs WHERE nid = ? ORDER BY id DESC";
        try (PreparedStatement ps = dbConn.prepareStatement(clubsSql)) {
            ps.setString(1, this.loggedInNid);
            ...
            showStatusMessage("Failed to load clubs data: " + e.getMessage(), true);
        }

        String eventsSql = "SELECT * FROM events WHERE nid = ? ORDER BY id DESC";
        try (PreparedStatement ps = dbConn.prepareStatement(eventsSql)) {
            ps.setString(1, this.loggedInNid);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
            ...
            showStatusMessage("Failed to load events data: " + e.getMessage(), true);
        }
    }
    ...
    private JPanel createManageClubsPanel() {
        ...
        addButton.addActionListener(e -> {
            ...
            if (dbConn != null) {
                String nid = this.loggedInNid;
                if (nid == null) {
                    showStatusMessage("Error: User NID not found. Cannot add club.", true);
                    return;
                }
    ...
    private JPanel createManageEventsPanel() {
        ...
        addButton.addActionListener(e -> {
            ...
            if (dbConn != null) {
                String nid = this.loggedInNid;
                if (nid == null) {
                    showStatusMessage("Error: User NID not found. Cannot add event.", true);
                    return;
                } 
                String abbreviation = (nid.length() >= 6) ? nid.substring(3, 6) : "def";
    ...
    /**
     * ✅ NEW, CORRECT 'show' METHOD
     * This method is called by LoginRegisterUI AFTER a successful login
     * to display the admin dashboard.
     * * @param parent The main application frame.
     * @param loggedInNid The NID of the admin who just logged in.
     */
    public static void show(JFrame parent, String loggedInNid) {
        SwingUtilities.invokeLater(() -> {
            // 1. Create an INSTANCE of the dashboard, passing the frame and NID
            AdminDashboard dashboard = new AdminDashboard(parent, loggedInNid);
            
            // 2. Get the dashboard's main panel (which was built in the constructor)
            Container dashboardPane = dashboard.getContentPane();
            
            // 3. Set the main frame's content to be the dashboard panel
            parent.getContentPane().removeAll();
            parent.getContentPane().add(dashboardPane);
            parent.revalidate();
            parent.repaint();
        });
    }
     ...
     * This method should be run to start your program.
     * It was previously named 'show', which caused the bug.
     */
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            rootFrame.setTitle("Nexclub Management");
            
            // The application starts by showing the login page
            LoginRegisterUI.show(rootFrame);
            
            // The LoginRegisterUI.show() method will handle maximizing the frame
