 ...
 */
public class CopyOfAdminDashboard3 {
    ...
    public CopyOfAdminDashboard3(JFrame parentFrame, String loggedInNid) { 
    ...
    private void fetchCollegeName() {
        if (dbConn == null || loggedInNid == null) return;
        if (loggedInNid.length() >= 6) {
            try (Statement st = dbConn.createStatement()) {
                String abbreviation = loggedInNid.substring(3, 6).toLowerCase();
    ...
    private void loadDataFromDB() {
        ...
        try (PreparedStatement ps = dbConn.prepareStatement(clubsSql)) {
            ...
            showStatusMessage("Failed to load clubs data.", true);
        }
        ...
        try (PreparedStatement ps = dbConn.prepareStatement(eventsSql)) {
            ...
            showStatusMessage("Failed to load events data.", true);
        }
    }
