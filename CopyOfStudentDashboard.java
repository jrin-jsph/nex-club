...
public class CopyOfStudentDashboard extends JFrame {
    ...
    private JPanel createSidebar() {
        ...
        logout.setForeground(accentBlue);
        logout.setBounds(14, 520, sidebarWidth - 28, 28);
        logout.addActionListener(e -> {
            new Logout();
            JOptionPane.showMessageDialog(this, "Logout placeholder instantiated.");
        });
        side.add(logout);
    ...
    private JPanel createDashboardPanel() {
        ...
        RoundedPanel pending = createStatCard(String.valueOf(pendingApplicationsCount), "Pending Applications", new Color(255,140,0));
        stats.add(joined); stats.add(eventsStat); stats.add(pending);
    ...
    private JPanel createBrowseClubsPanel() {
        ...
        for (Club c : myClubs) {
            ...
            JLabel lab = new JLabel("<html><b>" + c.name + "</b> &nbsp; <span style='color:gray;font-size:11px'>" + c.location + "</span></html>");
    ...
    private JPanel createClubCardForList(Club c) {
        ...
        JLabel title = new JLabel("<html><b>" + c.name + "</b></html>");
        applyAppFont(title, Font.BOLD, 13);
        JTextArea desc = new JTextArea(c.shortDesc);
        desc.setLineWrap(true);
        desc.setWrapStyleWord(true);
        desc.setOpaque(false);
    ...
    private JPanel createEventCardForList(EventItem e) {
        ...
        card.setBorder(new EmptyBorder(10,10,10,10));
        card.setPreferredSize(new Dimension(200,140));
        JLabel title = new JLabel("<html><b>" + e.title + "</b></html>");
        applyAppFont(title, Font.BOLD, 13);
        JLabel club = new JLabel(e.clubName);
        applyAppFont(club, Font.PLAIN, 12);
        club.setForeground(Color.GRAY);
        JTextArea info = new JTextArea("Venue: " + e.venue + " | Date: " + e.date + " | Time: " + e.time);
        info.setLineWrap(true);
        info.setWrapStyleWord(true);
        info.setOpaque(false);
        info.setEditable(false);
        applyAppFont(info, Font.PLAIN, 12);
        card.add(title, BorderLayout.NORTH);
        card.add(info, BorderLayout.CENTER);
        card.add(club, BorderLayout.SOUTH);
        card.addMouseListener(new MouseAdapter(){
    ...
    private JPanel createUpcomingEventRow(EventItem ev) {
        ...
        JLabel meta = new JLabel(ev.clubName + " | " + ev.date + " | " + ev.time);
    ...
    private void openClubPopup(Club c) {
        ...
        join.addActionListener(e -> {
            ...
            JOptionPane.showMessageDialog(this, "Joined " + c.name + " (static/database).");
            closePopup();
        });
    ...
    private void openEventPopup(EventItem ev) {
        ...
        apply.addActionListener(e -> {
            ...
            updateDashboardCounts();
            JOptionPane.showMessageDialog(this, "Applied to " + ev.title + " (static/database).");
            closePopup();
        });
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(() -> new CopyOfStudentDashboard());
    }
